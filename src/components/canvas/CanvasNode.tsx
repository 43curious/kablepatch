...
function CanvasNode({ node, geometry, selected, readOnly = false, connection, onPortDown, onPortUp, onPortClick }: Props) {
  ...
  return <g transform={`translate(${node.position.x} ${node.position.y})`} onPointerDown={e => { e.stopPropagation(); if (!readOnly) selectNode(node.id, e.shiftKey); }}>
    {compact ? <>
      ...
      <text x={compactInput ? w - 16 : 16} y={29} textAnchor={compactInput ? 'end' : 'start'} fill="#172033" fontSize={13} fontWeight={700} onDoubleClick={() => !readOnly && setRenaming(true)}>{node.label}</text>
      ...
      <rect x={0} y={switchNameStripeY(node)} width={w} height={40} fill={node.headerColor} onPointerDown={startDrag} />
      <text x={w / 2} y={switchNameStripeY(node) + 25} textAnchor="middle" fill="#fff" fontSize={14} fontWeight={700} onDoubleClick={() => !readOnly && setRenaming(true)}>{node.label}</text>
      ...
      <text x={10} y={22} fill="#fff" fontSize={13} fontWeight={700} onDoubleClick={() => !readOnly && setRenaming(true)}>{node.label}</text>
