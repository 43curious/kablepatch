...
function CanvasEdge({ edge, sourcePort, source, target, selected, dimmed, path }: Props) {
  ...
  if (!path) return <g className="edge edge-unroutable" onPointerDown={event => { event.stopPropagation(); selectEdge(edge.id); }}>
    ...
    {[source, target].map((point, i) => <circle key={i} cx={point.x} cy={point.y} r={7} fill="none" stroke="#ef4444" strokeWidth={2} strokeDasharray="3 2" />)}
  </g>;
  return <g className="edge">
    ...
    <path d={path} fill="none" stroke="#f8fafc" strokeWidth={6} strokeLinecap="round" strokeLinejoin="round" pointerEvents="none" />
    <path d={path} fill="none" stroke={dimmed ? '#94a3b8' : SIGNAL_TYPES[sourcePort.signalType].color} strokeOpacity={dimmed ? 0.25 : 0.9} strokeWidth={selected ? 3 : 2} strokeLinecap="round" strokeLinejoin="round" pointerEvents="none" />
