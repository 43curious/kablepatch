...
type SavedComponent = { id: string; label: string; category: string; emoji?: string; manualUrl?: string; inputGroups: PortGroup[]; outputGroups: PortGroup[]; bidirectionalGroups?: PortGroup[]; templateId?: string };
...
export default function FloatingToolbar({ svgRef, onOpenCanvas, onAutoAlign, projectId }: { svgRef: React.RefObject<SVGSVGElement | null>; onOpenCanvas: (document: CanvasDocument, merge?: boolean) => void; onAutoAlign: () => void; projectId?: string }) {
  const fileInput = useRef<HTMLInputElement>(null);
  const fileMode = useRef<'open' | 'import'>('open');
  const [q, setQ] = useState('');
  const [exportPassword, setExportPassword] = useState('');
  const [open, setOpen] = useState(true);
  const [tab, setTab] = useState<'library' | 'project' | 'export'>('library');
  const [libraryTab, setLibraryTab] = useState<'list' | 'new'>('list');
  const [projectTab, setProjectTab] = useState<'general' | 'inspector'>('general');
  ...
  const [portDrop, setPortDrop] = useState<{ side: 'input' | 'output' | 'bidirectional'; index: number } | null>(null);
  ...
  const selected = useCanvasStore(useShallow(state => {
    ...
    return node ? { id: node.id, label: node.label, category: node.category, emoji: node.emoji, catalogId: node.catalogId, ports: node.ports } : null;
  }));
  ...
  useEffect(() => {
    if (selectedNodeId && selectedNodeIds.length === 1) { setOpen(true); setTab('project'); setProjectTab('inspector'); }
  ...
  const current = (): SavedComponent => ({ id: savedId || crypto.randomUUID(), label: customLabel || 'Custom Device', category: customCategory, emoji: customEmoji || undefined, manualUrl: customManualUrl.trim() || undefined, inputGroups, outputGroups, bidirectionalGroups, templateId: templateId || undefined });
  const loadComponent = (id: string) => {
    if (!id) return resetForm();
    setSavedId(id);
    const c = saved.find(x => x.id === id);
    if (!c) return resetForm();
    setTemplateId(c.templateId ?? ''); setCustomLabel(c.label); setCustomCategory(c.category || 'Custom'); setCustomEmoji(c.emoji ?? ''); setCustomManualUrl(c.manualUrl ?? ''); setInputGroups(c.inputGroups); setOutputGroups(c.outputGroups); setBidirectionalGroups(c.bidirectionalGroups ?? []);
  };
  const resetForm = () => { setSavedId(''); setTemplateId(''); setCustomLabel('Custom Device'); setCustomCategory('Custom'); setCustomEmoji(''); setCustomManualUrl(''); setInputGroups([{ type: 'XLR', amount: 16, signalType: 'analog_audio' }]); setOutputGroups([{ type: 'XLR OUT', amount: 8, signalType: 'analog_audio' }]); setBidirectionalGroups([]); };
  const saveComponent = () => {
    ...
    setSavedId(c.id); setTab('library'); setLibraryTab('list'); setNotice(updating ? 'Library and canvas updated' : 'Added to library');
    setTimeout(() => setNotice(''), 1800);
  };
  const addCurrent = () => s.addCustomNode(customLabel, labels(inputGroups), labels(outputGroups), center(), customCategory, templateId || savedId || undefined, labels(bidirectionalGroups), customEmoji || undefined);
  ...
  const saveSelected = () => {
    if (!selected) return;
    const c = { id: crypto.randomUUID(), label: selected.label, category: selected.category || 'Custom', emoji: selected.emoji, manualUrl: saved.find(item => item.id === selected.catalogId || item.templateId === selected.catalogId)?.manualUrl, inputGroups: portGroups(selected.ports.filter(p => p.side === 'input')), outputGroups: portGroups(selected.ports.filter(p => p.side === 'output')), bidirectionalGroups: portGroups(selected.ports.filter(p => p.side === 'bidirectional')) };
    setSaved(xs => [...xs.filter(x => x.label !== c.label || x.category !== c.category), c]);
  };
  const editSaved = (c: SavedComponent) => {
    setTab('library'); setLibraryTab('new'); setSavedId(c.id); setTemplateId(c.templateId ?? ''); setCustomLabel(c.label); setCustomCategory(c.category || 'Custom'); setCustomEmoji(c.emoji ?? ''); setCustomManualUrl(c.manualUrl ?? ''); setInputGroups(c.inputGroups); setOutputGroups(c.outputGroups); setBidirectionalGroups(c.bidirectionalGroups ?? []);
  };
  ...
  return <div className={`side-stack ${open ? '' : 'sidebar-collapsed'}`}><aside className={`toolbar ${selected ? 'expanded' : ''} ${open ? '' : 'collapsed'}`}>
    <div className="sidebar-topbar">
      {open && <nav className="tabs" aria-label="Sidebar sections"><button className={tab === 'library' ? 'active' : ''} aria-current={tab === 'library' ? 'page' : undefined} onClick={() => { setTab('library'); setLibraryTab('list'); }}>Library</button><button className={tab === 'project' ? 'active' : ''} aria-current={tab === 'project' ? 'page' : undefined} onClick={() => { setTab('project'); setProjectTab('general'); }}>Project</button><button className={tab === 'export' ? 'active' : ''} aria-current={tab === 'export' ? 'page' : undefined} onClick={() => setTab('export')}>Export</button></nav>}
    ...
    {open && <>
      ...
      {tab === 'project' && <>
        <nav className="panel-subtabs" aria-label="Project tools"><button className={projectTab === 'general' ? 'active' : ''} aria-current={projectTab === 'general' ? 'page' : undefined} onClick={() => setProjectTab('general')}>General</button><button className={projectTab === 'inspector' ? 'active' : ''} aria-current={projectTab === 'inspector' ? 'page' : undefined} onClick={() => setProjectTab('inspector')}>Inspector</button></nav>
        {projectTab === 'general' && <div className="tab-panel project-panel">
        ...
        {projectTab === 'inspector' && <div className="component-inspector-panel">{selected ? <NodeInspector categories={categoryOptions} /> : <p className="empty-state">Select a component on the canvas to inspect its properties, ports, space, VLANs, and notes.</p>}</div>}
      </>}
      {false && tab === 'library' && libraryTab === 'new' && <div className="tab-panel custom-object">
          <label>Component icon<EmojiPicker value={customEmoji} onChange={setCustomEmoji} /></label>
        ...
        {tab === 'library' && <div className="tab-panel catalog">
          <input className="catalog-search" aria-label="Search catalog" placeholder="Search devices and presets…" value={q} onChange={e => setQ(e.target.value)} />
          <a className="manage-library-link" href={`/app/library/${projectId ? `?from=${encodeURIComponent(`/app/${projectId}/`)}` : ''}`}>Manage library <span aria-hidden="true">↗</span></a>
          <div className="node-list">
            {catalogSections.map(([section, categories]) => <details className="category-section" key={section} open><summary>{section}</summary><div className="category-list">
              {categories.map(cat => <details className="library-category" key={cat} open><summary style={{ color: CATEGORY_COLORS[cat] ?? CATEGORY_COLORS.Custom }}>{cat}</summary>
                {(savedGroups[cat] ?? []).sort((a, b) => a.label.localeCompare(b.label)).map(c => <div className="preset-row saved-preset" key={c.id}><button className="preset-add" onClick={() => s.addCustomNode(c.label, labels(c.inputGroups), labels(c.outputGroups), center(), c.category, c.templateId || c.id, labels(c.bidirectionalGroups ?? []), c.emoji)}><span aria-hidden="true">{c.emoji || '💾'}</span><span>{c.label}<small>{c.templateId ? 'Library override' : `${c.inputGroups.reduce((n, g) => n + g.amount, 0)} in · ${c.outputGroups.reduce((n, g) => n + g.amount, 0)} out`}</small></span></button><div className="preset-actions"><a href={specsUrl(c.label, c.manualUrl)} target="_blank" rel="noreferrer">Specs</a></div></div>)}
                {(groups[cat] ?? []).map(n => <div className="preset-row" key={n.id}><button className="preset-add" onClick={() => s.addNode(n.id, center())}><span aria-hidden="true">{n.icon}</span><span>{n.label}<small>Built-in device</small></span></button><div className="preset-actions"><a href={specsUrl(n.label)} target="_blank" rel="noreferrer">Specs</a></div></div>)}
              </details>)}
            </div></details>)}
