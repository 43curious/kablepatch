...
type SavedComponent = { id: string; label: string; category: string; inputGroups: PortGroup[]; outputGroups: PortGroup[]; templateId?: string };
...
export default function FloatingToolbar({ svgRef }: { svgRef: React.RefObject<SVGSVGElement | null> }) {
  ...
  const groups = useMemo(() => NODE_TYPES.filter(n => !hidden.includes(n.id) && !saved.some(c => c.templateId === n.id) && matches(n.label, n.category)).reduce<Record<string, typeof NODE_TYPES>>((a, n) => ((a[n.category] ??= []).push(n), a), {}), [q, hidden, saved]);
  ...
  const current = (): SavedComponent => ({ id: savedId || crypto.randomUUID(), label: customLabel || 'Custom Device', category: customCategory, inputGroups, outputGroups, templateId: templateId || undefined });
  const loadComponent = (id: string) => {
    if (!id) return resetForm();
    setSavedId(id);
    const c = saved.find(x => x.id === id);
    if (!c) return resetForm();
    setTemplateId(c.templateId ?? ''); setCustomLabel(c.label); setCustomCategory(c.category || 'Custom'); setInputGroups(c.inputGroups); setOutputGroups(c.outputGroups);
  };
  const resetForm = () => { setSavedId(''); setTemplateId(''); setCustomLabel('Custom Device'); setCustomCategory('Custom'); setInputGroups([{ type: 'XLR', amount: 16, signalType: 'analog_audio' }]); setOutputGroups([{ type: 'XLR OUT', amount: 8, signalType: 'analog_audio' }]); };
  const saveComponent = () => {
    const c = current(), updating = !!(savedId || templateId);
  ...
  const editSaved = (c: SavedComponent) => {
    setTab('create'); setSavedId(c.id); setTemplateId(c.templateId ?? ''); setCustomLabel(c.label); setCustomCategory(c.category || 'Custom'); setInputGroups(c.inputGroups); setOutputGroups(c.outputGroups);
  };
  ...
  const editTemplate = (n: typeof NODE_TYPES[number]) => {
    setTab('create'); setSavedId(''); setTemplateId(n.id); setCustomLabel(n.label); setCustomCategory(n.category); setInputGroups(templateGroups(n.defaultInputs)); setOutputGroups(templateGroups(n.defaultOutputs));
  };
    ...
    {open && <>
      ...
      {selected ? <><label>Port label layer<select value={s.portLayer} onChange={e => s.setPortLayer(Number(e.target.value))}><option value={0}>Base names</option>{Array.from({ length: aliasLayers }, (_, i) => <option key={i + 1} value={i + 1}>Alias layer {i + 1}</option>)}</select></label><div className="toolbar-actions"><button onClick={() => s.selectNode(null)}>← Catalog</button><button onClick={() => { const name = prompt('Space name'); if (name) s.addSpace(name); }}>+ Space</button><button onClick={s.autoAlign}>Auto align</button></div><NodeInspector /></> : <>
        ...
        {tab === 'create' && <div className="tab-panel custom-object">
          ...
          <div className="toolbar-actions form-actions"><button className="primary" onClick={addCurrent}>Add to canvas</button><button onClick={saveComponent}>{savedId || templateId ? 'Update preset' : 'Save preset'}</button><button onClick={resetForm}>New</button></div>
        </div>}
